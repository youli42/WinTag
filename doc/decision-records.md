...
- 决策范围：本次重构已完成并定稿的 8 项决策（D1 至 D8）
...
| F5 | **热键冲突处理** | 健壮性 | 未实现。`hotkey::register_all` 任一热键被占用即 `bail` 退出整个程序 | 中 | 改为逐个注册并记录失败项，冲突时降级（仅注册成功的热键）并打印提示，而非整体退出 |
| F6 | **用户配置（热键自定义/默认颜色）** | development-plan.md 阶段三未勾选项 | 未实现。热键与颜色均硬编码 | 低 | 后续引入配置文件时实现；当前无持久化设计，需先决策是否打破"纯会话"原则 |
