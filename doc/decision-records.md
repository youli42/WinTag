...
- **决策**：把 `ui/panel`（概览面板）、`ui/popup`（标签编辑弹窗）、`ui/settings`（设置页）、`ui/confirm`（退出确认）四个**自绘 Win32 窗口**迁至 [iced](https://github.com/iced-rs/iced)（`tiny-skia` 软件渲染器），用其声明式控件 + 主题 + 高 DPI 取代手写的 `WNDCLASSW`/`CreateWindowExW`/`WM_CTLCOLOR*`/`WM_DRAWITEM`/`BS_OWNERDRAW` 自绘/子类化/控件布局。**其余层面保持纯 Win32 不变**：
  ...
  - **机制收敛（总线最小化）**：为约束「跨线程/跨层接口不随迁入 iced 而膨胀」，D27 同时落地四条约束——① **注入设置器按层归属**：散落在两层的 7 个 `set_*` 中，`sys` 层 6 个——`sys/overlay.rs` 的 `set_tag_store`/`set_message_target`/`set_tooltip_theme`/`set_show_title`/`set_badge_always_top`，`sys/tray.rs` 的 `set_balloon_enabled`；`core` 层 1 个——`core/settings.rs` 的 `set_global_settings`（全局设置单例，属数据注入而非 UI 偏好）。sys 层按职责收敛为**一个 `NativePrefs` 纯值结构**（`show_title`/`badge_always_top`/`tooltip_theme`/`balloon_enabled`）+ **一个 `NativeBridge`**（封装覆盖层「上报 edit_tag / 激活窗口」的回传能力，替代 `set_message_target`；`set_tag_store` 数据本体另保留或并入），主线程一次性注入，消除「改一条 UI 偏好要同步改 N 个设置器」的牵连；② 主循环散落的 `try_recv`（tray×2 + iced×1）收进**单一 `pump_background_events(hwnd)` 阶段**，循环体读起来是「来源 → 动作」一张表；③ **托盘不绕 iced**：托盘是原生层、只与主线程对话，iced 只管理四个面板，二者不直连、iced 不拥有 overlay/tray/hotkey；④ iced 侧**不镜像 `WM_APP_*` 的 UI 消息**——原生消息只负责「进主循环」，出主循环一律走 iced 通道，同一条消息只有一个出口、不双写。
