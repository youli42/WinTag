...
│   ├── layout.rs    # DPI 缩放辅助（dp()，D11）+ 客户区高度折算（client_height()/TITLEBAR_H，D25：外高抵扣标题栏边框，底部控件定位基准）
