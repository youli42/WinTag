...
| 标签与便签UI | 左上角圆边三角形角标 + 标题条（超 5 字省略号），悬停展开完整便签，快捷键快速添加 | ✅ 已完成 |
| 全局概览面板 | 快捷键呼出纵向树形列表（`标题 \| 窗口名称`，展开看完整多行备注），单击置前目标窗口，标签变更自动刷新，搜索过滤 | ✅ 已完成 |
...
标记窗口后，目标窗口左上角会出现橙色角标与标题条（可在设置中开关），提示该窗口已被标记；悬停角标可查看完整标题与备注。

标记弹窗内支持完整键盘操作：`回车` 保存（备注框中同样生效）、`Shift+回车` 备注内换行、`Esc` 取消、`Tab`/`Shift+Tab` 循环切换焦点。

## 测试

项目包含 63 个测试（30 个冒烟测试 + 33 个单元测试），覆盖核心数据层、配置持久化、主题解析、热键逻辑、WinEvent 事件分类与角标渲染纯函数：
